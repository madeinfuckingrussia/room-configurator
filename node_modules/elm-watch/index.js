...
var BEGIN_SYNC_UPDATE = "\x1B[?2026h";
var END_SYNC_UPDATE = "\x1B[?2026l";
...
function webSocketParamsDecodeError(error, urlParams) {
  return `
...
var WebSocketServer = class {
  constructor(portChoice, webSocketToken) {
...
function error401(socket, message) {
    ...
    case "CompileAllOutputsAsNeeded": {
      ...
      switch (cmd.mode) {
        case "AfterInstallDependencies":
          logger.withSynchronizedOutput(() => {
            printStatusLinesForElmJsonsErrors(
              logger,
              mutable.project
            );
...
function parseWebSocketConnectRequestUrl(project, urlParams) {
  ...
  if (webSocketConnectedParams.elmWatchVersion !== "1.2.6") {
    return {
      tag: "WrongVersion",
...
function webSocketConnectRequestUrlErrorToString(error) {
  switch (error.tag) {
    case "ParamsDecodeError":
      return webSocketParamsDecodeError(error.error, error.urlParams);
    case "WrongVersion":
...
function printEventMessage(event) {
  switch (event.tag) {
    ...
    case "WebSocketConnectionRejected": {
      const origin = event.origin === void 0 ? "unknown origin" : quote(event.origin);
...
function printWebSocketConnectionRejectedReason(reason) {
  switch (reason.tag) {
    case "BadUrl":
...
// src/Run.ts
async function run3(cwd, env, logger, getNow, runMode, args, restartReasons, postprocessWorkerPool, webSocketState, webSocketTokenFromPreviousRun, hotKillManager) {
                  ...
                  switch (runMode) {
                    ...
                    case "hot": {
                      const result = await run(
                        ...
                        postprocessWorkerPool,
                        webSocketState,
                        elmWatchStuffJson2?.webSocketToken ?? webSocketTokenFromPreviousRun ?? markAsWebSocketToken(crypto3.randomUUID()),
                        project,
...
async function handleElmWatchJsonError(logger, getNow, runMode, elmWatchJsonPath, postprocessWorkerPool, webSocketTokenFromPreviousRun) {
  switch (runMode) {
    case "make":
      return { tag: "Exit", exitCode: 1 };
    case "hot": {
        ...
        const doIt = async () => {
          ...
          do {
            result = await run3(
              ...
              result === void 0 ? void 0 : result.webSocketToken,
              hotKillManager
            );
